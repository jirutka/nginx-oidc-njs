 ...
 */
export function requestToken (ctx: Context, grantType: GrantType, value: string): Promise<TokenResponse> {
  ...
  return subrequest('POST', `${conf.internalLocationsPrefix}/request-token`, {
 ...
 */
export function refreshToken (ctx: Context, encryptedRefreshToken: string): Promise<TokenResponse> {
  const { conf } = ctx
 ...
 * @param accessToken
 */
export function verifyToken (ctx: Context, accessToken: string): Promise<Required<TokenInfo>> {
  const { conf, vars } = ctx

  return fetchTokenInfo(ctx, accessToken).then(token => {
...
function fetchTokenInfo (ctx: Context, token: string): Promise<TokenInfo> {
  const { conf, subrequest } = ctx

  return subrequest('POST',
